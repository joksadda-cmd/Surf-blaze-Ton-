...
export function verifyTelegramInitData(initData) {
    ...
    try {
        ...
        // 🔒 SECURITY — timing-safe comparison instead of `!==`. A plain
        // string comparison exits as soon as it finds the first mismatched
        // character, so in theory the response time leaks how many leading
        // hex characters an attacker's guess got right, letting a hash be
        // brute-forced one character at a time over enough requests.
        // timingSafeEqual always takes the same amount of time regardless
        // of where (or whether) the strings differ. Both sides must be the
        // same length for it to work, hence the explicit length check.
