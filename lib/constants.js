...
export const GAME_COINS_TO_DC = 1;              // ⚠️ CHANGED — was 500. The in-game drop-coin VALUE *is* the DC reward now, no multiplier — what the "Drops collected" screen shows is exactly what gets added to dcBalance (see api/earn.js). Kept as a named constant in case you ever want to reintroduce a multiplier later.
export const GAME_MAX_COINS_PER_SECOND = 0.05;  // sanity ceiling — blocks a forged/huge coins value
export const GAME_MAX_REWARD_DC = 20;           // ⚠️ CHANGED — was 1500 (that was sized for the old 500x multiplier). Rescaled to match the new 1:1 economy — a very long, very lucky run tops out around here.
...
// Both methods now pay out in USDT. Withdraw spends straight from
// usdtBalance (already-converted USDT) — no rate lookup needed at
// withdraw time; the DC_PER_USD conversion happens earlier, at Convert.
