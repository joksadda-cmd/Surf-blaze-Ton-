...
// ══════════════════════════════════════════════════════════
// WITHDRAW — impression-only (⚠️ CHANGED). A user can no longer withdraw
// DC directly — they must first use Convert (below) to turn DC into
// impressions, then withdraw FROM their impressionBalance:
//   usdAmount = (impressions / 1000) * currentCpmRate
// See api/withdraw.js. The old "type a DC amount, it's converted to
// impressions on the spot" flow is gone — MIN_WITHDRAW_DC no longer exists,
// only MIN_WITHDRAW_IMPRESSIONS matters here now.
//
...
// ══════════════════════════════════════════════════════════
// CONVERT — DC → impression (⚠️ NEW). This is now the ONLY way to get
// impressions: a user explicitly converts a chunk of their DC balance,
// pays a flat CONVERT_FEE_PERCENT fee (taken out of the DC itself — the fee
// portion is simply not converted, it's gone, same as any exchange fee),
// and the remainder becomes impressions at the fixed DC_PER_IMPRESSION
// ratio. Limited to CONVERT_DAILY_LIMIT successful conversions per
// Bangladesh calendar day (see todayBD()). See api/convert.js.
// ══════════════════════════════════════════════════════════
...
export const CONVERT_FEE_PERCENT = 5;    // taken out of the DC being converted, not out of the resulting impressions
