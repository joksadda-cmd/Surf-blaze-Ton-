...
//
// ⚠️ FIXED-RATE ECONOMY — the old impression/CPM system (DC → impressions
// at a fixed ratio, then impressions → USD via an admin-set weekly CPM
// rate) is GONE. Replaced with a single fixed rate, same as the app's
// task-reward pricing already used:
//   • Users earn DC (Drop Coin) from tasks/games/etc.
//   • DC converts straight to USDT balance at the fixed DC_PER_USD rate
//     (see api/convert.js) — no admin-set rate, no impressions unit.
//   • A flat CONVERT_FEE_PERCENT fee is taken out of the DC at convert
//     time — nothing further is taken on withdraw itself (api/withdraw.js
//     just spends straight from usdtBalance).

export const CURRENCY = 'DC';         // Drop Coin — Surf Blaze Ton's currency

// ✅ Your Mini App's own Vercel domain — single source of truth so it's
// only ever set in one place. Used by api/bot.js (web_app buttons, the
// broadcast worker fetch) and by lib/telegram.js (self-hosted theme images
// — see COVER_PHOTO / PAYMENT_PROOF_PHOTO below) alike.
export const APP_URL = 'https://surf-blaze-ton.vercel.app';
...
// ── DC → USDT fixed rate ── 50,000 DC = $1 USDT. Used by Convert
// (api/convert.js), Withdraw (api/withdraw.js, display/audit only), the
// admin panel (api/bot.js), and mirrored in index.html for client-side
// display. This is the ONLY exchange rate in the app now — no admin-set
// CPM, no impressions unit.
export const DC_PER_USD = 50000;

// ⚠️ Unrelated to the withdraw/convert economy above — this is just a fixed
...
export const GAME_COINS_TO_DC = 1.15;           // ⚠️ Admin requested ~15% reward increase — was 1 (flat 1:1). Kept as a named constant in case you ever want to change the multiplier again.
export const GAME_MAX_COINS_PER_SECOND = 0.05;  // sanity ceiling — blocks a forged/huge coins value
export const GAME_MAX_REWARD_DC = 23;           // ⚠️ Admin requested ~15% reward increase — was 20 (itself already rescaled once from 1500, back when the old 500x multiplier existed). This is the ceiling a very long/lucky run can reach — raised alongside GAME_COINS_TO_DC above so long runs (500m+) keep seeing their reward climb instead of visibly flatlining at the old 20 cap.
...
// Withdraw spends straight from usdtBalance (already-converted USDT) — no
// rate lookup needed at withdraw time; the DC_PER_USD conversion happens
// earlier, at Convert.
// ⚠️ REMOVED — Binance UID withdraw method fully removed per admin
// decision. Tonkeeper (TON wallet address) is now the only payout method.
export const WITHDRAW_METHODS = {
    tonkeeper: { label: 'Tonkeeper Address', currency: 'USDT', minCurrency: 0.1 },
};

// ══════════════════════════════════════════════════════════
// WITHDRAW — USDT-direct. A user can no longer withdraw DC directly — they
// must first use Convert (below) to turn DC into USDT balance at the fixed
// DC_PER_USD rate, then withdraw FROM usdtBalance. See api/withdraw.js.
//
// ⚠️ REMOVED — the weekly-Friday-only submission window (was
// WITHDRAW_DAY_ONLY_FRIDAY + isFridayBD()) has been fully removed per admin
// request. Withdrawals can now be submitted any day, any time (subject to
// the other requirements below — level, tasks, ads, referral — and the
// WITHDRAWALS_OPEN master switch).
// ══════════════════════════════════════════════════════════
export const MIN_WITHDRAW_USDT = 1;      // minimum USDT amount you can withdraw in one request

// ══════════════════════════════════════════════════════════
// CONVERT — DC → USDT. This is the ONLY way to get USDT balance: a user
// explicitly converts a chunk of their DC balance, pays a flat
// CONVERT_FEE_PERCENT fee (taken out of the DC itself — the fee portion is
// simply not converted, it's gone, same as any exchange fee), and the
// remainder becomes USDT at the fixed DC_PER_USD rate. Limited to
// CONVERT_DAILY_LIMIT successful conversions per Bangladesh calendar day
// (see todayBD()). See api/convert.js.
// ══════════════════════════════════════════════════════════
export const CONVERT_MIN_DC = 500;       // smallest DC amount you can convert in one go
export const CONVERT_MAX_DC = 100000;    // largest DC amount you can convert in one go
export const CONVERT_FEE_PERCENT = 5;    // taken out of the DC being converted, not out of the resulting USDT
export const CONVERT_DAILY_LIMIT = 1;    // conversions allowed per Bangladesh calendar day
...
// ── Level / XP system ──
// ⚠️ NEW — admin-requested anti-abuse layer: watching ads earns XP, XP adds
// up to a level (capped at MAX_LEVEL), and reaching WITHDRAW_MIN_LEVEL is a
// withdrawal requirement (alongside the ads/tasks gates above). The point
// is friction that only accrues through genuine time-and-usage — a
// script/Termux user hammering the API can't shortcut XP the way they might
// otherwise script around a same-day ads/tasks count. Exact XP-per-ad and
// the threshold table are a first pass — admin said more specific
// requirements may follow, so everything is centralized here to retune
// later without touching any other file.
export const XP_PER_AD_WATCH = 10;
export const MAX_LEVEL = 10;
export const WITHDRAW_MIN_LEVEL = 3;
// index 0 = XP needed to be AT level 1 (0, everyone starts here), index 1 =
// XP needed to REACH level 2, etc. Length must equal MAX_LEVEL.
export const LEVEL_XP_THRESHOLDS = [0, 50, 120, 220, 350, 520, 730, 990, 1300, 1700];

// Given total XP, returns { level, xp, xpForCurrentLevel, xpForNextLevel } —
// xpForNextLevel is null at MAX_LEVEL (nothing further to show progress
// toward). Used by api/withdraw.js (gate check) and api/user.js (profile
// display) so both always agree on the same math.
export function levelFromXp(xp) {
    ...
    const xpForCurrentLevel = LEVEL_XP_THRESHOLDS[level - 1];
    const xpForNextLevel = level < MAX_LEVEL ? LEVEL_XP_THRESHOLDS[level] : null;
...
// WITHDRAWN DC-EQUIVALENT AMOUNT (usdtAmount × DC_PER_USD, computed at
// withdraw time — see api/withdraw.js dcEquivalent) directly to their own
// dcBalance — e.g. a withdrawal equivalent to 10,000 DC pays the
