// api/bot.js — SURF BLAZE Admin Panel Bot
//
//   1) ADMIN_ID env var — matches the Vercel env var name for this project
//   2) In-memory state{} — persisted in MongoDB (lib/adminState.js), since
//      in-memory state can be lost on Vercel serverless cold starts
//   3) Uses _id instead of a telegramId field (matches our users schema)
//   4) Single currency: dcBalance
//   5) All video-watch admin actions (🎬 Add Video / Manage Videos) removed —
//      that feature doesn't exist in the Mini App anymore.
//   6) The 🚩 Multi-Account Flags admin panel (device-cluster review,
//      auto-suspend-the-rest-on-tap, "Make Main") is REMOVED.
//      Enforcement happens live at the door via lib/ipRegistry.js (device
//      fingerprint, IP only as a fallback — see that file), not via a manual
//      admin queue.
//   7) A rejected withdraw fully refunds the USDT (real money was never
//      sent) — see api/withdraw.js for the USDT-direct withdraw itself,
//      and api/convert.js for the DC → USDT Convert step (fixed
//      DC_PER_USD rate in lib/constants.js, minus a flat Convert fee).
//   8) 👥 All Users, 📅 Weekly Refer, and 📜 Weekly Report removed from the
//      admin menu — not part of this build.
//   9) ✅ Verify filter removed from 🗑 Manage Tasks (Exclusive/Partner/
//      Earning/All only) — the Add Task "Channel/Group Join" category
//      itself is untouched, this only removed the Manage Tasks list filter.

import { ObjectId } from 'mongodb';
import { connectToDatabase } from '../lib/mongodb.js';
import { tgApi, tgSend, tgEdit, tgSendPhoto, tgSendPhotoSafe, tgAnswerCallback, isMember, OFFICIAL_CHANNEL, COMMUNITY_GROUP, PAYMENT_PROOF_PHOTO } from '../lib/telegram.js';
...
import { DC_PER_USD_TASK_REWARD, WITHDRAW_REFERRAL_COMMISSION_PERCENT, DC_PER_USD, APP_URL } from '../lib/constants.js';
import { markBanned, markUnbanned } from '../lib/banRegistry.js';

// Small formatter shared by the Manage Tasks listings — shows the USDT
// figure alongside its converted DC amount for any old rows still priced
// in USDT (that option is no longer offered in Add Task), plain DC otherwise.
function taskRewardLine(t) {
    return t.rewardCurrency === 'usdt' ? `${t.rewardUsdt} USDT (≈${t.rewardDc} DC)` : `${t.rewardDc} DC`;
}
...
// APP_URL itself lives in lib/constants.js (single source of truth — see the import above).
const MINI_APP_URL = 'https://t.me/SurfBlazeTON_bot/SurfPlay';
const BOT_USERNAME = 'SurfBlazeTON_bot';
const COVER_PHOTO = `${APP_URL}/cover.png`; // ✅ self-hosted on your own Vercel domain now — see cover.png at the project root
// Was previously hotlinked to postimg.cc — no longer any third-party
// image-host dependency. cover.png (and payment-proof.png, used below via
// PAYMENT_PROOF_PHOTO in lib/telegram.js) is a generated water/sky-themed
// "SURF BLAZE" banner, served as a plain static file alongside index.html
// and game.html. If you ever redesign it, just replace the PNG file at the
// project root — same filename, no code change needed. tgSendPhotoSafe is
// still used below as a defensive fallback in case static hosting is ever
// misconfigured.
...
const adminKb = {
    inline_keyboard: [
        ...
        [{ text: '📋 Add Task', callback_data: 'a_addtask' }, { text: '🗑 Manage Tasks', callback_data: 'a_managetasks_menu' }],
        ...
        [{ text: '💰 Send DC', callback_data: 'a_senddc' }, { text: '🎁 Send Gift', callback_data: 'a_sendgift' }],
    ],
};
...
function renderUserLookup(u, withdrawalsList = []) {
    ...
    const totalWithdrawnDc = withdrawalsList.filter(w => w.status === 'approved').reduce((sum, w) => sum + (w.dcEquivalent ?? 0), 0);
    ...
    const text =
        ...
        `💰 Balance: <b>${u.dcBalance || 0} DC</b>\n` +
        `💎 Lifetime Earned: <b>${u.lifetimeDcEarned || 0} DC</b>\n` +
                `✅ Tasks Completed: <b>${(u.completedTasks || []).length}</b>\n` +
        `👥 Referrals: <b>${u.referralCount || 0}</b>\n` +
        `📤 Withdrawals: <b>${wCount}</b> (${pendingWithdrawCount} pending) — <b>${totalWithdrawnDc.toLocaleString()} DC</b> approved lifetime\n` +
    ...
    const options = { reply_markup: { inline_keyboard: [
        ...
        [{ text: '👥 View Their Referrals', callback_data: `a_refslist_${u._id}_0` }, { text: '💰 Send DC', callback_data: `quickdc_${u._id}` }],
...
export default async function handler(req, res) {
    ...
    // ══════════════════════════════════════════════════════════
    // 🔒 CRITICAL SECURITY GATE — verify this request actually came from
    // Telegram's servers, not a forged payload POSTed directly to this URL.
    //
    // Without this check, ANYONE who knows (or finds/guesses) your numeric
    // ADMIN_ID could send a raw HTTP request here (curl, Termux, browser
    // devtools — no Telegram account needed) with a fake body like
    // { message: { from: { id: <ADMIN_ID> }, text: "..." } }, and every
    // "fromId === ADMIN_ID" check further down in this file would trust it
    // completely — full admin takeover: send themselves unlimited DC,
    // approve their own withdrawals, unban themselves,
    // anything the admin panel can do. This was a REAL gap, not theoretical.
    //
    // The fix: Telegram signs every genuine webhook request with a secret
    // token YOU chose when registering the webhook, sent in the
    // X-Telegram-Bot-Api-Secret-Token header. We just need to check it
    // matches — Telegram is the only party that will ever send it correctly.
    //
    // ⚠️ SETUP REQUIRED — this check only works once you've:
    //   1) Set a TELEGRAM_WEBHOOK_SECRET env var on Vercel (any long random
    //      string — e.g. generate one with `openssl rand -hex 32`).
    //   2) Re-registered your webhook WITH that secret, by visiting this URL
    //      once in a browser (replace the placeholders):
    //      https://api.telegram.org/bot<YOUR_BOT_TOKEN>/setWebhook?url=<YOUR_APP_URL>/api/bot&secret_token=<YOUR_TELEGRAM_WEBHOOK_SECRET>
    //   Until both are done, every request is rejected below (fails closed,
    //   not open — the bot simply won't respond rather than staying exposed).
    // ══════════════════════════════════════════════════════════
    const expectedSecret = process.env.TELEGRAM_WEBHOOK_SECRET;
    if (!expectedSecret) {
        console.error('TELEGRAM_WEBHOOK_SECRET is not set — rejecting all webhook requests until it is configured. See the comment above this check.');
    ...
    // ⚠️ NEW — the entire rest of this handler used to run with no top-level
    // try/catch. Any unhandled error anywhere below (a bad MONGODB_URI, a
    // Telegram API call failing, any bug) crashed the function completely
    // silently — no reply, no error to the admin, nothing — which looks
    // EXACTLY like "the bot just stopped responding" from the outside, with
    // zero clue why. Wrapping it means: (1) the admin gets a Telegram
    // message telling them something broke and what the error was, instead
    // of just silence, and (2) it's now impossible for one bad update to
    // permanently wedge the bot — every request still gets a 200 response.
    try {
    ...
    async function finalizeWithdrawal({ w, approve, txHash, chatId, msgId }) {
        ...
        if (!approve) {
            // Withdrawals deduct straight from `usdtBalance` (only ever
            // filled by Convert — see api/convert.js). A rejected
            // withdrawal refunds that same `usdtAmount` back to usdtBalance.
            ...
            const refundUpdate = { $inc: { usdtBalance: w.usdtAmount || 0, withdrawalCount: -1 }, $set: { lastWithdrawDate: '', withdrawPending: false } };
        ...
        // Referral withdrawal commission only fires on actual approval,
        // here — a rejected withdrawal never pays a commission in the
        // first place, nothing to claw back. 10% of the withdrawn USDT's
        // DC-equivalent (dcEquivalent, set at request time — see
        // api/withdraw.js), paid directly as DC.
        let referrerCommission = 0;
        if (approve && w.referrerId) {
            referrerCommission = Math.floor((w.dcEquivalent || 0) * (WITHDRAW_REFERRAL_COMMISSION_PERCENT / 100));
        ...
        const notif = approve
            ? `🎉 <b>Congratulations! Your payment request has been approved.</b> ✅\n\n` +
              `Check the details below:\n\n` +
              `💵 Amount: <b>${w.cashAmount.toFixed(4)} ${w.currency}</b>\n` +
              `📍 Address: <code>${w.details}</code>\n\n` +
              `💪 Keep up the great work! Complete tasks and refer your friends to earn even more DC every day. 🚀`
            : `❌ <b>Withdrawal Rejected.</b>\n$${(w.usdtAmount || 0).toFixed(4)} USDT has been refunded to your balance.`;
        // ⚠️ CHANGED — no more public "proof" channel post at all (removed
        // PAYMENT_CHANNEL entirely, per request). The approval now goes
        // straight to the user as ONE photo DM: the payment-approved banner
        // (payment-proof.png) with the full amount/address details as its
        // caption, instead of a separate text message + a separate public
        // channel photo. Rejections stay plain text (no photo needed).
        const notifButtons = [];
        ...
        if (approve) {
            await tgSendPhotoSafe(w.userId, PAYMENT_PROOF_PHOTO, notif, notifExtra);
        } else {
        ...
        const processedText =
            ...
            (w.usdtAmount != null ? `💰 Amount: <b>$${w.usdtAmount.toFixed(4)}</b>${w.dcEquivalent ? ` (≈ ${w.dcEquivalent.toLocaleString()} DC)` : ''}\n` : '') +
            `💰 Amount: <b>${w.cashAmount.toFixed(4)} ${w.currency}</b>\n` +
            `📤 Method: <b>${w.method}</b>\n` +
            `📍 Address: <code>${w.details}</code>\n` +
    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('check_join_')) {
            ...
            await tgSendPhotoSafe(chatId, COVER_PHOTO,
                `✅ <b>Verified! Welcome to SURF BLAZE!</b>\n\n🌊 Earn DC · Withdraw crypto!`,
                { reply_markup: { inline_keyboard: [
                    [{ text: '🚀 Open SURF BLAZE', web_app: { url: APP_URL } }],
                    [{ text: '👥 Share & Earn', url: `https://t.me/share/url?url=${encodeURIComponent(MINI_APP_URL + '?startapp=' + userId)}&text=${encodeURIComponent('🌊 Join SURF BLAZE! Earn DC crypto!')}` }],
                ] } }
            );
            return res.status(200).json({ ok: true });
        }
        ...
        if (data.startsWith('wd_addrconfirm_')) {
            ...
            const usdtAmount = w.usdtAmount || 0;
            const penalty = usdtAmount / 2;
            const refund = usdtAmount - penalty;

            // Same lock-release + first-withdraw/referral refund logic as a
            // normal reject (see finalizeWithdrawal in the admin section
            // below) — just with only HALF the USDT actually going back.
            const refundUpdate = { $inc: { usdtBalance: refund, withdrawalCount: -1 }, $set: { lastWithdrawDate: '', withdrawPending: false } };
            if (w.referralConsumed) refundUpdate.$inc.usedValidReferrals = -1;
            await users.updateOne({ _id: w.userId }, refundUpdate);
            ...
            await tgEdit(chatId, msgId,
                `✅ <b>Confirmed.</b>\n\n` +
                `💰 Refunded to your balance: <b>$${refund.toFixed(4)} USDT</b>\n` +
                `⚠️ Penalty kept: <b>$${penalty.toFixed(4)} USDT</b>\n\n` +
            ...
            if (w.adminMsgChatId && w.adminMsgId) {
                tgEdit(
                    ...
                    `🪙 Refunded: <b>${refund.toLocaleString()} DC</b> · Penalty: <b>${penalty.toLocaleString()} DC</b>\n` +
                    `📅 ${new Date().toLocaleString()}`,
                    { reply_markup: { inline_keyboard: [] } }
                ).catch(() => {});
            } else if (ADMIN_ID) {
                tgSend(ADMIN_ID, `⚠️ Wrong-address penalty applied for <code>${w.userId}</code> — refunded ${refund.toLocaleString()} DC, kept ${penalty.toLocaleString()} DC.`).catch(() => {});
            }
        ...
        if (data.startsWith('wd_wrongaddr_')) {
            ...
            await tgSend(
                ...
                `Your withdrawal address doesn't look right for the method you chose. SURF BLAZE only accepts:\n` +
                ...
                `💰 Amount: <b>$${(w.usdtAmount || 0).toFixed(4)} USDT</b>\n\n` +
                `You can confirm below to close this request now — <b>50% of the USDT will be refunded to your balance, and 50% will be kept as a penalty</b> for submitting an invalid address. Please double-check your address next time.`,
        ...
        // ── Confiscate balance — zeroes out spendable DC (the actual punishment for fraud:
        ...
        if (data.startsWith('zerobal_')) {
            ...
            const u = await users.findOne({ _id: target });
            if (!u) { await tgEdit(chatId, msgId, '❌ User not found.', { reply_markup: backKb }); return res.status(200).json({ ok: true }); }
            const seized = u.dcBalance || 0;
            await users.updateOne({ _id: target }, { $set: { dcBalance: 0 } });
            await tgEdit(chatId, msgId, `💸 Confiscated <b>${seized.toLocaleString()} DC</b> from <code>${target}</code> — balance is now 0.`, { reply_markup: { inline_keyboard: [[{ text: '👤 View User', callback_data: `lookup_${target}` }], [{ text: '◀️ Back to Menu', callback_data: 'a_menu' }]] } });
            return res.status(200).json({ ok: true });
        }
        ...
        // ── Quick "Send DC" shortcut from the user detail panel — target already known,
        // so it skips straight to asking for the amount. ──
        if (data.startsWith('quickdc_')) {
            const target = data.replace('quickdc_', '');
            await setAdminState(fromId, { step: 'senddc_amount', targetId: target });
            await tgEdit(chatId, msgId, `💰 How much <b>DC</b> do you want to send to <code>${target}</code>? (negative number to deduct)`, { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
        ...
        if (data === 'a_menu') {
            await clearAdminState(fromId);
            await tgEdit(chatId, msgId, '👑 <b>SURF BLAZE Admin Panel</b>\n\nSelect an option:', { reply_markup: adminKb });
            return res.status(200).json({ ok: true });
        }
        ...
        if (data === 'a_stats') {
            ...
            const dcAgg = await users.aggregate([{ $group: { _id: null, t: { $sum: '$dcBalance' } } }]).toArray();
            await tgEdit(chatId, msgId,
                `📊 <b>Dashboard</b>\n\n` +
                ...
                `💰 Total DC (all users): <b>${(dcAgg[0]?.t || 0).toLocaleString()}</b>\n` +
                `💵 Rate: <b>${DC_PER_USD.toLocaleString()} DC = $1</b> (fixed)`,
                { reply_markup: backKb }
            );
            return res.status(200).json({ ok: true });
        }
        ...
        // ⚠️ Article/Faucet/Video task types removed (this update) — the
        // shortcut handlers that used to live here (a_addarticle /
        // a_addfaucet / a_addvideo / a_managevideos / delvideo_) are gone.
        // Use "📋 Add Task" → pick a category instead.
        ...
        // ── 🗑 Manage Tasks — category picker (Exclusive/Partner/Earn/All) ──
        ...
        if (data.startsWith('a_viewpromos_')) {
            ...
            list.forEach(p => {
                ...
                out += `<code>${p.code}</code> — ${status}\n` +
                       `   💰 ${p.reward} DC · 👥 Claimed: <b>${used}</b>${max ? ` / ${max}` : ' (unlimited)'}\n` +
        ...
        if (data === 'a_senddc') {
            await setAdminState(fromId, { step: 'senddc_id' });
            await tgEdit(chatId, msgId, '💰 Send the <b>Telegram ID</b> of who you want to send DC to:', { reply_markup: cancelKb });
            return res.status(200).json({ ok: true });
        }
    ...
    if (text.startsWith('/start')) {
            ...
            if (fromId === targetId) { // verify they're appealing for their own account — prevents spamming with someone else's ID
                ...
                if (ADMIN_ID) {
                    await tgSend(ADMIN_ID,
                        ...
                        `💰 Balance: <b>${targetUser?.dcBalance || 0} DC</b>\n` +
        ...
        if (fromId === String(ADMIN_ID)) {
            ...
            await tgSend(chatId, `👑 <b>SURF BLAZE Admin Panel</b>\n\nWelcome back, Admin!`, { reply_markup: adminKb });
            return res.status(200).json({ ok: true });
        }
        ...
        if (!ch || !com) {
            await tgSendPhotoSafe(chatId, COVER_PHOTO,
                `🌊 <b>Welcome to SURF BLAZE!</b>\n\n` +
                `Earn free crypto (DC → TON/USDT) — no investment required! 💰\n\n` +
        ...
        await tgSendPhotoSafe(chatId, COVER_PHOTO,
            ...
            `👥 Refer friends for bonus DC\n` +
            `🎁 Completely free — no investment required\n\n` +
            `👇 Tap the button below to get started!`,
            { reply_markup: { inline_keyboard: [
                [{ text: '🚀 Open SURF BLAZE', web_app: { url: APP_URL } }],
                [{ text: '👥 Share & Earn', url: `https://t.me/share/url?url=${encodeURIComponent(MINI_APP_URL + '?startapp=' + fromId)}&text=${encodeURIComponent('🌊 Join SURF BLAZE! Earn DC crypto!')}` }],
            ] } }
        );
        return res.status(200).json({ ok: true });
    }
    ...
    const s = await getAdminState(fromId);
    if (!s) {
        await tgSend(chatId, '👑 <b>SURF BLAZE Admin Panel</b>', { reply_markup: adminKb });
        return res.status(200).json({ ok: true });
    }
    ...
    if (s.step === 'user_lookup') {
        ...
        if (!/^\d+$/.test(text)) {
            ...
            matches.forEach((u, i) => {
                out += `<b>${i + 1}.</b> ${u.firstName || 'User'} (@${u.telegramUsername || 'n/a'})\n   ID: <code>${u._id}</code> | 💰 ${u.dcBalance || 0} DC | 👥 ${u.referralCount || 0} refs | 📤 ${u.withdrawalCount || 0} withdraws${u.isBanned ? ' 🔒' : ''}\n\n`;
            });
    ...
    // ── Send DC ──
    if (s.step === 'senddc_id') {
        s.targetId = text; s.step = 'senddc_amount';
        await setAdminState(fromId, s);
        await tgSend(chatId, `💰 How much <b>DC</b> do you want to send to <code>${text}</code>?`);
        return res.status(200).json({ ok: true });
    }
    if (s.step === 'senddc_amount') {
        ...
        await clearAdminState(fromId);
        if (!u) { await tgSend(chatId, '❌ User not found.', { reply_markup: adminKb }); return res.status(200).json({ ok: true }); }
        await users.updateOne({ _id: s.targetId }, { $inc: { dcBalance: amt, lifetimeDcEarned: amt } });
        await tgSend(chatId, `✅ <b>${amt} DC</b> sent to <code>${s.targetId}</code>`, { reply_markup: adminKb });
        await tgSend(s.targetId, `🎁 You've received <b>${amt} DC</b> from the admin!`);
        return res.status(200).json({ ok: true });
    }
    ...
    if (s.step === 'promo_reward') {
        ...
        await setAdminState(fromId, s);
        await tgSend(chatId, `Reward: <b>${s.reward} DC</b>\n\nMax uses per code? (0 = unlimited):`);
        return res.status(200).json({ ok: true });
    }
    ...
    if (s.step === 'task_reward') {
        ...
        if (s.rewardCurrency === 'usdt') {
            s.rewardUsdt = val;
            s.reward = Math.round(val * DC_PER_USD_TASK_REWARD); // converted once here — everything downstream (earn.js reward crediting) just sees plain DC
        } else {
    ...
    if (s.step === 'task_quantity') {
        ...
        s.step = 'task_confirm';
        await setAdminState(fromId, s);
        const rewardLine = s.rewardCurrency === 'usdt' ? `<b>${s.rewardUsdt} USDT</b> (≈ ${s.reward} DC)` : `<b>${s.reward} DC</b>`;
    ...
    if (s.step === 'task_confirm') {
        ...
        await tasks.insertOne({
            ...
            rewardDc: s.reward, rewardCurrency: s.rewardCurrency || 'dc', rewardUsdt: s.rewardUsdt || null,
            limit: s.limit, isApproved: true, completionCount: 0, createdAt: new Date(),
        });
        const doneRewardLine = s.rewardCurrency === 'usdt' ? `${s.rewardUsdt} USDT (≈${s.reward} DC)` : `${s.reward} DC`;
    ...
    res.status(200).json({ ok: true });
        ...
        // Something above threw. Log it (visible in Vercel → Project →
        // Functions logs) and tell the admin directly in Telegram instead of
        // just going silent — this is the single biggest debugging upgrade
        // for this file: next time something breaks, you'll get a message
        // telling you the actual error instead of having to guess.
        console.error('api/bot.js handler error:', err);
        if (ADMIN_ID) {
            const detail = (err && err.message) ? err.message : String(err);
            await tgSend(ADMIN_ID, `⚠️ <b>Bot error</b>\n\n<code>${detail.slice(0, 500)}</code>`).catch(() => {});
        }
        // Always 200 back to Telegram — a 4xx/5xx here just makes Telegram
        // retry the same update repeatedly, which usually makes things worse.
