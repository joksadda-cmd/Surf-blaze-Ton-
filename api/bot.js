...
//      admin queue.
//   7) A rejected withdraw fully refunds impressions (including the 5%
//      Convert fee that was already paid — since real money was never
//      sent; see api/convert.js and api/withdraw.js for the DC→impression
//      Convert step and the impression-only withdraw itself)
...
//   10) 📈 Set CPM — NEW admin action. The withdraw payout economy (see
//       lib/constants.js + api/withdraw.js) now runs on a CPM rate the
//       admin sets here at any time (stored in the `settings` collection —
//       see lib/settings.js), instead of a fixed DC→USD rate.
...
import { DC_PER_USD_TASK_REWARD, WITHDRAW_REFERRAL_COMMISSION_PERCENT, DC_PER_IMPRESSION, MIN_WITHDRAW_IMPRESSIONS, APP_URL } from '../lib/constants.js';
import { getCpmRate, setCpmRate } from '../lib/settings.js';
...
// ⚠️ MINI_APP_URL / BOT_USERNAME are placeholders — replace with your
// actual @bot_username once you create the bot with @BotFather (they must
// match each other exactly, or deep-links break). APP_URL itself now lives
// in lib/constants.js (single source of truth — see the import above).
const MINI_APP_URL = 'https://t.me/YOUR_BOT_USERNAME/YOUR_APP_SHORTNAME'; // ⚠️ REPLACE — e.g. https://t.me/SurfBlazeTon_bot/App
const BOT_USERNAME = 'YOUR_BOT_USERNAME';                                 // ⚠️ REPLACE — must match MINI_APP_URL — used to build the appeal deep-link
...
const adminKb = {
    inline_keyboard: [
        ...
        [{ text: '📈 Set CPM', callback_data: 'a_setcpm' }],
...
function renderUserLookup(u, withdrawalsList = []) {
    ...
    const totalWithdrawnDc = withdrawalsList.filter(w => w.status === 'approved').reduce((sum, w) => sum + (w.dcEquivalent ?? w.dcAmount ?? 0), 0);
...
export default async function handler(req, res) {
    ...
    async function finalizeWithdrawal({ w, approve, txHash, chatId, msgId }) {
        ...
        if (!approve) {
            // ⚠️ CHANGED — withdrawals now deduct from `impressionBalance`
            // (see api/withdraw.js), not `dcBalance` directly — a user must
            // Convert DC → impressions first (api/convert.js, 5% fee) before
            // they can withdraw at all. A rejected withdrawal refunds that
            // same `impressions` amount back to impressionBalance — the
            // user gets their impressions back in full (the 5% Convert fee
            // they already paid earlier is NOT re-refunded here; it was a
            // separate, already-completed transaction).
            ...
            const refundUpdate = { $inc: { impressionBalance: w.impressions || 0, withdrawalCount: -1 }, $set: { lastWithdrawDate: '', withdrawPending: false } };
        ...
        // ⚠️ CHANGED — commission is 10% of `dcEquivalent` (impressions ×
        // DC_PER_IMPRESSION), the DC value the withdrawn impressions
        // represent — since impressions themselves aren't a currency the
        // referrer can be paid in. Still paid out as real DC.
        ...
        const notif = approve
            ...
            : `❌ <b>Withdrawal Rejected.</b>\n${(w.impressions || 0).toLocaleString()} impressions have been refunded to your balance.`;
        ...
        const processedText =
            ...
            (w.impressions ? `📊 Impressions: <b>${w.impressions.toLocaleString()}</b>${w.dcEquivalent ? ` (≈ ${w.dcEquivalent.toLocaleString()} DC)` : ''}\n` : (w.dcAmount ? `🪙 DC: <b>${w.dcAmount.toLocaleString()}</b>\n` : '')) +
            `💰 Amount: <b>${w.cashAmount.toFixed(4)} ${w.currency}</b>\n` +
            `📤 Method: <b>${w.method}</b>\n` +
            `📍 Address: <code>${w.details}</code>\n` +
    ...
    if (update.callback_query) {
        ...
        if (data.startsWith('wd_addrconfirm_')) {
            ...
            const impressionsAmount = w.impressions || 0;
            const penalty = Math.floor(impressionsAmount / 2);
            const refund = impressionsAmount - penalty;

            // Same lock-release + first-withdraw/referral refund logic as a
            // normal reject (see finalizeWithdrawal in the admin section
            // below) — just with only HALF the impressions actually going
            // back (⚠️ CHANGED — impressionBalance now, not dcBalance).
            const refundUpdate = { $inc: { impressionBalance: refund, withdrawalCount: -1 }, $set: { lastWithdrawDate: '', withdrawPending: false } };
            if (w.referralConsumed) refundUpdate.$inc.usedValidReferrals = -1;
            await users.updateOne({ _id: w.userId }, refundUpdate);
            ...
            await tgEdit(chatId, msgId,
                `✅ <b>Confirmed.</b>\n\n` +
                `📊 Refunded to your balance: <b>${refund.toLocaleString()} impressions</b>\n` +
                `⚠️ Penalty kept: <b>${penalty.toLocaleString()} impressions</b>\n\n` +
        ...
        if (data.startsWith('wd_wrongaddr_')) {
            ...
            await tgSend(
                ...
                `📊 Amount: <b>${(w.impressions || 0).toLocaleString()} impressions</b>\n\n` +
                `You can confirm below to close this request now — <b>50% of the impressions will be refunded to your balance, and 50% will be kept as a penalty</b> for submitting an invalid address. Please double-check your address next time.`,
        ...
        if (data === 'a_stats') {
            ...
            await tgEdit(chatId, msgId,
                ...
                `📈 Current CPM: <b>$${cpmRate}</b> / 1K impressions`,
                { reply_markup: backKb }
            );
            return res.status(200).json({ ok: true });
        }

        // ── 📈 Set CPM — the weekly (or whenever) admin-adjusted payout rate.
        // See lib/settings.js / lib/constants.js for how this feeds into the
        // withdraw calculation: usdAmount = (impressions/1000) * cpmRate.
        if (data === 'a_setcpm') {
            const cpmRate = await getCpmRate(db);
            await setAdminState(fromId, { step: 'setcpm_value' });
            await tgEdit(chatId, msgId,
                `📈 <b>Set CPM</b>\n\n` +
                `Current rate: <b>$${cpmRate}</b> per 1,000 impressions\n` +
                `(${DC_PER_IMPRESSION.toLocaleString()} DC = 1 impression · minimum withdraw = ${MIN_WITHDRAW_IMPRESSIONS.toLocaleString()} impressions)\n\n` +
                `Send the new CPM rate in USD (e.g. <code>0.2</code> or <code>0.5</code>):`,
    ...
    // ── Set CPM ──
    if (s.step === 'setcpm_value') {
        const val = parseFloat(text);
        if (!val || isNaN(val) || val <= 0) { await tgSend(chatId, '❌ Enter a valid positive number (e.g. 0.2):'); return res.status(200).json({ ok: true }); }
        await clearAdminState(fromId);
        await setCpmRate(db, val, fromId);
        await tgSend(chatId,
            `✅ <b>CPM updated!</b>\n\n📈 New rate: <b>$${val}</b> per 1,000 impressions\n\n` +
            `Example: a user withdrawing ${MIN_WITHDRAW_IMPRESSIONS.toLocaleString()} impressions (${(MIN_WITHDRAW_IMPRESSIONS * DC_PER_IMPRESSION).toLocaleString()} DC) now gets $${((MIN_WITHDRAW_IMPRESSIONS / 1000) * val).toFixed(4)}.`,
