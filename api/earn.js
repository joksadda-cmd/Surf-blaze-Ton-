...
// before the ad SDK is even called. No reward yet — just a signature the
// client must carry through the real ad flow and hand back to
// claimAdReward. A script that skips straight to claimAdReward with no
// token (or a stale/forged one) is rejected outright.
//
// 🔒 SECURITY — same single-active-token pattern as gameStart/claimGameReward
// (see there for the full "mint many tokens, wait, claim them all"
// exploit writeup). For ads specifically, the per-network daily counter +
// AD_COOLDOWN_SECONDS between claims ALREADY hard-caps total possible
// earnings regardless of how many tokens are pre-minted — so this isn't
// closing an actual unlimited-earning hole here, just removing the
// "claim without any token having been freshly issued for this specific
// claim" gap for consistency/defense-in-depth.
async function handleAdStart(req, res, db, userId) {
    ...
    await db.collection('users').updateOne({ _id: userId }, { $set: { [`currentAdStartTime.${network}`]: startTime } });
    ...
    const gate = await users.findOneAndUpdate(
        {
            ...
            [`currentAdStartTime.${network}`]: Number(startTime),
            $or: [{ lastAdClaimAt: { $exists: false } }, { lastAdClaimAt: { $lte: cooldownCutoff } }],
            ...REWARD_ELIGIBLE_FILTER,
        },
        {
            $inc: { dcBalance: config.reward, lifetimeDcEarned: config.reward, lifetimeAdsWatched: 1, adsWatchedToday: 1, xp: XP_PER_AD_WATCH, [counterField]: 1 },
            $addToSet: { usedAdStarts: adStartKey },
            $set: { lastAdClaimAt: new Date(), [`currentAdStartTime.${network}`]: null },
        },
        { returnDocument: 'after' }
    );

    if (!gate) {
        const exists = await users.findOne({ _id: userId }, { projection: { isBanned: 1, [counterField]: 1, usedAdStarts: 1, lastAdClaimAt: 1, multiAccountFlag: 1, channelVerified: 1, currentAdStartTime: 1 } });
        ...
        if ((exists.currentAdStartTime || {})[network] !== Number(startTime)) return res.status(400).json({ ok: false, error: 'ad_token_expired' });
...
// adStart: no reward yet — just a signature the game overlay's host carries
// through the run and hands back to claimGameReward when the player crashes.
//
// 🔒 SECURITY — only ONE outstanding (unclaimed) game token per user at a
// time. Without this, a script could call gameStart in a tight loop to mint
// hundreds of valid signed tokens instantly, wait an hour, then claim EACH
// one for the maximum reward its elapsed-time cap allows — multiplying
// their payout by however many tokens they minted, since nothing tied a
// token's reward to actual concurrent, non-overlapping gameplay. Storing
// the single currently-valid startTime on the user doc and requiring
// claimGameReward's startTime to match it exactly closes that: minting a
// new token immediately invalidates whatever token came before it.
async function handleGameStart(req, res, db, userId) {
    if (!SECRET) return res.status(500).json({ ok: false, error: 'server_misconfigured' });
    const startTime = Date.now();
    await db.collection('users').updateOne({ _id: userId }, { $set: { currentGameStartTime: startTime } });
...
// collected/travelled, the token itself is single-use, AND (see
// handleGameStart above) it must be the MOST RECENTLY issued token for this
// user — an older, superseded token is rejected even if its signature is
...
async function handleClaimGameReward(req, res, db, userId) {
    ...
    //
    // ⚠️ CHANGED — GAME_COINS_TO_DC is now 1 (coins reported BY the game
    // already ARE the DC reward, e.g. "Drops collected: 0.1254" → +0.1254
    // DC, exactly). Rounding to a whole number here used to matter when a
    // 500x multiplier made rewards land in the tens/hundreds; now that
    // real per-run rewards are small fractions (0.001–0.3 range per pickup),
    // Math.round() would have silently zeroed out almost every legitimate
    // reward. Rounded to 4 decimal places instead — matches the precision
    // the game client already reports coins at.
    ...
    const rewardDc = Math.min(GAME_MAX_REWARD_DC, Math.round(Math.max(0, cappedCoins * GAME_COINS_TO_DC) * 10000) / 10000);
    ...
    //
    // 🔒 SECURITY — `currentGameStartTime` must equal this exact `startTime`
    // (set by handleGameStart above, re-set on every new gameStart call).
    // This is what actually stops the "mint many tokens, wait, claim them
    // all" exploit described above handleGameStart: only the single most
    // recently issued token can ever satisfy this filter, so minting a new
    // one immediately and permanently invalidates every older one — no
    // amount of waiting lets a stale token accumulate a bigger reward.
    ...
    const gate = await users.findOneAndUpdate(
        {
            ...
            currentGameStartTime: Number(startTime),
            ...REWARD_ELIGIBLE_FILTER,
        },
        {
            ...
            $set: { currentGameStartTime: null },
        },
        { returnDocument: 'after' }
    );

    if (!gate) {
        const exists = await users.findOne({ _id: userId }, { projection: { isBanned: 1, usedGameStarts: 1, multiAccountFlag: 1, channelVerified: 1, currentGameStartTime: 1 } });
        if (!exists) return res.status(404).json({ ok: false, error: 'user_not_found' });
        if (exists.isBanned) return res.status(403).json({ ok: false, error: 'banned' });
        if ((exists.usedGameStarts || []).includes(gameStartKey)) return res.status(400).json({ ok: false, error: 'game_token_already_used' });
        if (exists.currentGameStartTime !== Number(startTime)) return res.status(400).json({ ok: false, error: 'game_token_expired' });
        if (exists.multiAccountFlag && !exists.channelVerified) return res.status(403).json({ ok: false, error: 'account_under_review' });
        return res.status(400).json({ ok: false, error: 'claim_failed' });
    }
